...
describe('POST /api/transactions (type 4) register multisignature', function () {
	...
	var scenarios = {
		...
		'regular_signature_in_place': new shared.MultisigScenario(),
	...
	describe('transactions processing', function () {
		...
		it('with all the signatures already in place (regular_signature_in_place scenario) should fail', function () {
			transaction = node.lisk.multisignature.createMultisignature(scenarios.regular_signature_in_place.account.password, null, scenarios.regular_signature_in_place.keysgroup, scenarios.regular_signature_in_place.lifetime, scenarios.regular_signature_in_place.min);
			...
			return sendTransactionPromise(transaction)
				.then(function (res) {
					...
					scenarios.regular_signature_in_place.transaction = transaction;

					return node.Promise.map(scenarios.regular_signature_in_place.members, function (member) {
						signature = node.lisk.multisignature.signTransaction(scenarios.regular_signature_in_place.transaction, member.password);

						return sendSignaturePromise(signature, scenarios.regular_signature_in_place.transaction).then(function (res) {
							node.expect(res).to.have.property('statusCode').to.equal(apiCodes.OK);
							node.expect(res).to.have.nested.property('body.status').to.equal('Signature Accepted');
						});
					});
				...
				.then(function () {
					goodTransactions.push(scenarios.regular_signature_in_place.transaction);
					signature = node.lisk.multisignature.signTransaction(scenarios.regular_signature_in_place.transaction, scenarios.regular_signature_in_place.members[0].password);

					return sendSignaturePromise(signature, scenarios.regular_signature_in_place.transaction);
				})
				.then(function (res) {
